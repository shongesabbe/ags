...
## Quick Start
* The following instructions are for building the Engine and Editor from the raw files on GitHub. If you want to skip building, you can download the current version of the installer here [AGS Installer](www.adventuregamestudio.co.uk/site/ags/)
...
**IMPORTANT:** all libraries should match the Engine's architecture: e.g. if you are building the engine using 32-bit (x86) configuration then link libraries for 32-bit (x86) too.
...
The reason for this is that the Engine project itself links C runtime statically.

### SDL2

For Windows you may download ready lib and DLL files directly [from SDL2 homepage](https://www.libsdl.org/download-2.0.php). You need the one under "Development Libraries", archive is called SDL2-devel-2.x.x-VC.zip (where x.x is the latest version number).
...
AGS engine will need the following files to link in order to function:
...
For reference, at the time of the last update, our build server is using the following revision: https://github.com/icculus/SDL_sound/archive/c5639414c1bb24fb4eef5861c13adb42a4aab950.zip
...
If you are running CMake from the command line, you may also pass this path as an argument, for example:
...
The Engine requires the following libraries:
* SDL2
* SDL_Sound
* libogg
* libtheora
...
If you prefer to build everything yourself: you need to build *static* libraries compiled with **/MT** or **/MTd** option (*statically* linked runtime C library). This is important or you may get linking errors, as the engine itself is compiled with **/MT(d)**.
...
In order to direct Studio to necessary libraries and their headers, setup the following environment variables in your system by [creating user macros in the Property Pages](https://docs.microsoft.com/en-us/cpp/build/working-with-project-properties?view=msvc-160#user-defined-macros):
...
The Editor has two related solutions: Solutions\AGS.Editor.Full.sln and Solutions\AGS.Editor.NoNative.sln. The latter is useful if your version of MSVS cannot build mixed (C#/C++) assemblies, or if you specifically do not want to work with and/or compile C++ code.
...
If you are working with AGS.Editor.NoNative solution then you won't have to deal with the parts written in C++, but you will have to get compatible compiled AGS.Native.dll somewhere. We suggest using one from the latest release of AGS.
...
The manual way is to run the compiler from the command-line and pass these as arguments, for example:
...
In any case, there are a number of files that have to be prepared for the installer to actually build. These files have to be placed in Windows\Installer\Source and subdirectories:
